    ...
    def register(self) -> None:
        ...
        def quiz_page() -> None:
            ...
            with ui.column().classes("w-full items-center p-8 gap-6 bg-gray-50"):
                ui.label("🎯 QuizRP").classes("text-5xl font-bold text-center bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent")
                ui.label("Answer all questions to get your grade (1-6)").classes("text-xl text-gray-600 text-center mb-4")
                with ui.column().classes("w-full max-w-2xl gap-6"):
                    with ui.card().classes("w-full shadow-md bg-white").style("border-radius: 12px; border: 1px solid #e5e7eb;"):
                        name_input = ui.input("Your name").classes("w-full text-lg").props("outlined rounded")
                    with ui.card().classes("w-full sticky top-4 z-10 shadow-lg bg-white").style("border-radius: 12px; border: 2px solid #ddd;"):
                    ...
                    for index, question in enumerate(questions, start=1):
                        with ui.card().classes("w-full p-6 shadow-md bg-white").style("border-radius: 12px; border: 1px solid #d1d5db;"):
                            with ui.row().classes("items-center gap-4 mb-3"):
                                ui.label(str(index)).classes("text-2xl font-bold bg-blue-500 text-white rounded-full w-10 h-10 flex items-center justify-center")
                            ...
                            ui.radio(options, on_change=lambda e, qid=question.id: on_select(qid, e.value)).props("color=blue").classes("text-gray-800")
                    refresh_progress()
                    ...
                    def do_submit() -> None:
                        ...
                        with result_container:
                            ...
                            with ui.card().classes(f"w-full p-8 {bg} shadow-xl").style(f"border-radius: 16px; border: 2px solid; border-color: {border};"):
                                ...
                                ui.label(f"Correct: {attempt.num_correct}/{attempt.num_questions}").classes("text-lg text-gray-900")
                                ui.label(f"Score: {attempt.score_percent:.1f}%").classes("text-lg text-gray-900")
                                ui.label(f"Grade: {attempt.grade}").classes("text-4xl font-bold text-blue-600 my-4")
                                status = "✅ PASSED" if attempt.grade >= 4.0 else "❌ NOT PASSED"
                                ui.label(status).classes("text-2xl font-bold text-gray-900")
                                ui.button("Try again", icon="refresh", on_click=lambda: ui.navigate.to("/")).props("rounded").classes("mt-4 bg-blue-600")
                    ui.button("Submit Quiz", icon="send", on_click=do_submit).props("size=lg rounded").classes("w-full bg-blue-600 shadow-lg")
                    ui.link("Admin Panel →", "/admin").classes("text-blue-700 text-center mt-4 font-semibold")
        @ui.page("/admin")
        def admin_page() -> None:
            with ui.column().classes("w-full items-center p-8 bg-gray-50"):
                ...
                with ui.column().classes("w-full max-w-5xl"):
                    ...
                    for attempt in attempts:
                        local_time = attempt.created_at.replace(tzinfo=timezone.utc).astimezone(ZoneInfo("Europe/Zurich"))
                        rows.append({
                            "id": attempt.id,
                    ...
                    with ui.card().classes("w-full shadow-lg bg-white").style("border-radius: 12px; border: 1px solid #d1d5db;"):
